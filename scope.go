...
func (scope *Scope) AddToVars(value interface{}) string {
	...
	if _, ok := value.(time.Time); !ok && (reflect.ValueOf(value).Kind() == reflect.Map || reflect.ValueOf(value).Kind() == reflect.Slice || reflect.ValueOf(value).Kind() == reflect.Struct) {
		tempValueBytes, _ := json.Marshal(value)

		scope.SQLVars = append(scope.SQLVars, string(tempValueBytes))
	} else {
...
func (scope *Scope) scan(rows *sql.Rows, columns []string, fields []*Field) {
		...
		for fieldIndex, field := range selectFields {
			if field.DBName == column {
				if field.Field.Kind() == reflect.Ptr {
					...
					kind := field.Struct.Type.Kind()
					_, ok := field.Field.Interface().(time.Time)
					if !ok && (kind == reflect.Map || kind == reflect.Slice || kind == reflect.Struct) {
						values[index] = &TipuSqlScanner{Type: field.Struct.Type, Value: field.Field}
					} else {
						reflectValue := reflect.New(reflect.PtrTo(field.Struct.Type))
						reflectValue.Elem().Set(field.Field.Addr())
						reflectValueInterface := reflectValue.Interface()
						values[index] = reflectValueInterface
					}
	...
	_ = scope.Err(rows.Scan(values...))

	for index, field := range resetFields {
		value, ok := values[index].(*TipuSqlScanner)
		if ok {
			field.Field.Set(value.Value)
		} else if v := reflect.ValueOf(values[index]).Elem().Elem(); v.IsValid() {
			field.Field.Set(v)
		}
	}
}
...
func (scope *Scope) createTable() *Scope {
	var tags []string // tags存放可以被创建的的字段
	...
	for _, field := range scope.GetModelStruct().StructFields {
		if field.IsNormal {
			...
			//检查是否将主键约束指定为字段类型的一部分。如果是，只能支持一个字段作为主键
